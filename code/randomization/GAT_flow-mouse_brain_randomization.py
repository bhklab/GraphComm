...
#     matrix_list.append(pd.read_csv(f"/h/soemily/GAT/data/GSE95025/GSM249478{j}_dge_mel_rep{i}.txt",sep="\t",index_col=0))
#     if not common_genes:
#         common_genes = pd.read_csv(f"/h/soemily/GAT/data/GSE95025/GSM249478{j}_dge_mel_rep{i}.txt",sep="\t",index_col=0).index.tolist()
#     else:
#         common_genes = list(set(common_genes) & set(pd.read_csv(f"/h/soemily/GAT/data/GSE95025/GSM249478{j}_dge_mel_rep{i}.txt",sep="\t",index_col=0).index.tolist()))
...
matrix = pd.read_csv("/h/soemily/new_GAT/data/GraphComm_Input/GSE133079/matrix.csv",index_col=0)
...
for i in range(len(df_list)):
    temp = df_list[i]
    temp = temp.head(1000)
    intercell = pd.read_csv("/h/soemily/new_GAT/data/LR_database/intercell_Omnipath.csv")
    temp = pd.merge(temp,intercell,left_on=["Src","Dst"],right_on=["source","target"])
    temp = temp.head(1000)
    temp.to_csv(f"/h/soemily/GAT/results/GSE133079/intercell_Mouse_{i+91}_random.csv")
