...
def get_cell_LR_embeddings(matrix,meta,nodes,interactions,total_embeddings_df,Omnipath_nodes,Omnipath_interactions,spatial=None,reproduce=None,save=None,cell_lr=0.001):
    device = 'cpu'
    torch.manual_seed(0)
    truth_info = pd.DataFrame(np.zeros((Omnipath_nodes.shape[0],Omnipath_nodes.shape[0])),index=Omnipath_nodes["identifier"].tolist(),columns=Omnipath_nodes["identifier"].tolist())
    ident_interactions = Omnipath_interactions.copy()
    ...
    for i in receptors:
        full_matrix.loc[i][ligands] = (gene_mean.loc[ligands]*gene_mean.loc[i]).tolist()
    print("continuing...")
    ...
    for i in receptors:
        full_matrix.loc[i][ligands] *= total_out_df[i][ligands]
    ...
    temp = full_matrix.loc[ligands+receptors][ligands+receptors]
    temp = temp.replace(-0.0, 0.0)
    df = temp.values
    normalized_df=(df-np.min(df))/(np.max(df)-np.min(df))
    full_matrix.loc[ligands+receptors,ligands+receptors] = normalized_df
    
    matrix_ligands = [i.split("_")[0] for i in ligands]
    matrix_receptors = [i.split("_")[0] for i in receptors]
    
    LR_cell_df = pd.DataFrame(np.zeros((len(ligands)+len(receptors),len(cell_groups))),index=ligands+receptors,columns=cell_groups)
    
    ligand_matrix = matrix.loc[matrix_ligands]
    ligand_matrix = ligand_matrix[~ligand_matrix.index.duplicated(keep='first')]
    receptors_matrix = matrix.loc[matrix_receptors]
    receptors_matrix = receptors_matrix[~receptors_matrix.index.duplicated(keep='first')]
    ligand_matrix.index = ligands
    receptors_matrix.index = receptors
    
    for i in cell_groups:
        cells = meta[meta["labels"]==i]["cell"].tolist()
        LR_cell_df.loc[ligands,i] = ligand_matrix.loc[ligands][cells].mean(axis=1)
        LR_cell_df.loc[receptors,i] = receptors_matrix.loc[receptors][cells].mean(axis=1)
    
    temp = LR_cell_df
    df = temp.values
    normalized_df=(df-np.min(df))/(np.max(df)-np.min(df))
    LR_cell_df[:] = normalized_df

    full_matrix.loc[[str(i) for i in cell_groups],ligands+receptors] = LR_cell_df.transpose().values
    full_matrix.loc[ligands+receptors,[str(i) for i in cell_groups]] = LR_cell_df.values
    ...
    if reproduce is not None:
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=cell_lr)
        ...
        for epoch in range(100):
            ...
            if epoch % 20 == 0:
                print(f"Epoch:{epoch} Null Loss:{loss}")
            loss.backward()
    ...
    ligand_probs = ligand_out[:,1].cpu().detach().numpy()
    receptor_probs = receptor_out[:,1].cpu().detach().numpy()
    ligand_probs = (ligand_probs-np.min(ligand_probs))/(np.max(ligand_probs)-np.min(ligand_probs))
    receptor_probs = (receptor_probs-np.min(receptor_probs))/(np.max(receptor_probs)-np.min(receptor_probs))
    total_out = np.array([i*receptor_probs for i in ligand_probs])
